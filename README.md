...
```sh
make docker-build docker-push IMG=<some-registry>/host-operator:tag
```
...
```sh
make deploy IMG=<some-registry>/host-operator:tag
```
...
```sh
make build-installer IMG=<some-registry>/host-operator:tag
```
...
```sh
kubectl apply -f https://raw.githubusercontent.com/DanNiESh/host-operator/<tag or branch>/dist/install.yaml
```
