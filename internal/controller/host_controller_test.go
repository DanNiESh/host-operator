...
var _ = Describe("Host Controller", func() {
	Context("When reconciling a resource", func() {
		...
		BeforeEach(func() {
			By("creating the custom resource for the Kind Host")
			err := k8sClient.Get(ctx, typeNamespacedName, host)
			if err != nil && errors.IsNotFound(err) {
				resource := &osacopenshiftiov1alpha1.Host{
		...
		AfterEach(func() {
			...
			By("Cleanup the specific resource instance Host")
			Expect(k8sClient.Delete(ctx, resource)).To(Succeed())
		})
		It("should successfully reconcile the resource", func() {
			By("Reconciling the created resource")
			controllerReconciler := &HostReconciler{
				Client: k8sClient,
